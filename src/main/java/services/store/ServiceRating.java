...
public class ServiceRating {
    ...
    public void supprimerRatingProd(String ref) throws SQLException {
        Connection connection = DataBase.getInstance().getConnection();

        // Définir la requête SQL pour supprimer les enregistrements liés à la référence du produit
        String req = "DELETE FROM ratingprod WHERE ref = ?";
        PreparedStatement ps = connection.prepareStatement(req);
        ps.setString(1, ref);
        ps.executeUpdate();
        System.out.println("Produit supprimé de rating");
